...
async def editar_musica(
    ...
    return templates.TemplateResponse("criar_musica.html", {"request": request, "message": "Música atualizada com sucesso!"})
...
async def excluir_musica(request: Request, musica_id: int, db: Session = Depends(get_db)):
    musica_repository = MusicaRepository(db)
    musica_service = MusicaService(musica_repository)
    musica_service.excluir_musica(musica_id)
    musicas = musica_repository.obter_todas()
    return templates.TemplateResponse("index.html", {"request": request, "musicas": musicas, "message": "Música excluída com sucesso!"})
